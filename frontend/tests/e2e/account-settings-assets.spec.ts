...
test('desktop: prompt workspace supports complete variables and template duplication', async ({ page }) => {
  ...
  await expect(duplicateDialog.getByRole('button', { name: '插入变量 角色名' })).toBeVisible()
  await expect(duplicateDialog.getByRole('button', { name: '插入变量 场景地点' })).toBeVisible()
  await expect(duplicateDialog.getByRole('button', { name: '插入变量 道具名称' })).toBeVisible()
