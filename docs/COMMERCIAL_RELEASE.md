...
- 已有按直连来源 IP 的全 API 固定窗口限流，以及组织级每日任务/活跃任务配额；公开服务仍需按厂商价格计算的金额预算、代理信任策略及滥用检测；
...
- 已实现组织级写操作审计日志、组织 JSON 导出和带双重确认的组织删除主路径；导出查询错误和媒体删除异常已显式失败；正式发布仍需补外部媒体删除补偿/重试，并定义审计留存周期、可恢复备份流程和隐私政策；
