...
func TestUpdateAndBindingValidationBranches(t *testing.T) {
	...
	tests := []struct {
		...
		{http.MethodPut, "/api/v1/agent-configs/" + idText(agentConfig.ID), `{"max_iterations":6}`, http.StatusBadRequest},
		...
		{http.MethodPost, "/api/v1/agent-configs", `{"agent_type":"extractor","max_iterations":6}`, http.StatusBadRequest},
		{http.MethodPost, "/api/v1/agent-configs", `{"agent_type":"extractor","unknown":true}`, http.StatusBadRequest},
		{http.MethodPost, "/api/v1/agent-configs", `{"agent_type":"extractor","name":"` + strings.Repeat("x", maxNameRunes+1) + `"}`, http.StatusBadRequest},
		{http.MethodPost, "/api/v1/agent-configs", `{"agent_type":"extractor","system_prompt":"` + strings.Repeat("x", maxTextRunes+1) + `"}`, http.StatusBadRequest},
		{http.MethodPost, "/api/v1/agent-configs", `{"agent_type":"voice_assigner","name":"Voice"}`, http.StatusOK},
		{http.MethodPost, "/api/v1/agent/script_rewriter/chat", `{"drama_id":` + idText(drama.ID) + `,"episode_id":` + idText(episode.ID) + `,"message":"x","unknown":true}`, http.StatusBadRequest},
		{http.MethodPost, "/api/v1/agent/script_rewriter/chat", `{"drama_id":` + idText(drama.ID) + `,"episode_id":` + idText(episode.ID) + `,"message":"` + strings.Repeat("x", maxTextRunes+1) + `"}`, http.StatusBadRequest},
